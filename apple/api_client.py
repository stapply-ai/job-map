...
class Job:
    ...
    # Detailed job fields (fetched separately)
    description: Optional[str] = None
    minimumQualifications: Optional[str] = None
    preferredQualifications: Optional[str] = None
    payAndBenefits: Optional[str] = None
    ...
    def full_description(self) -> str:
        """Merge all description fields into one comprehensive description."""
        parts = []

        if self.description:
            parts.append(self.description)
        elif self.jobSummary:
            parts.append(self.jobSummary)

        if self.minimumQualifications:
            parts.append(f"\n\nMinimum Qualifications:\n{self.minimumQualifications}")

        if self.preferredQualifications:
            parts.append(f"\n\nPreferred Qualifications:\n{self.preferredQualifications}")

        if self.payAndBenefits:
            parts.append(f"\n\nPay & Benefits:\n{self.payAndBenefits}")
...
class AppleJobsAPI:
    ...
    def get_job_details(self, job: Job) -> Job:
        """
        Fetch detailed job information from the API.

        This method fetches the full job details from the API and extracts:
        - Full description
        - Minimum qualifications
        - Preferred qualifications
        - Pay & benefits (if available)
        ...
        Example:
            >>> job = jobs[0]
            >>> detailed_job = client.get_job_details(job)
            >>> print(detailed_job.full_description)
        """
        try:
            logger.debug(f"Fetching details for job {job.positionId}")
            ...
            # Some jobs may have pay and benefits, check for it
            # The field might be in different formats depending on the job
            if 'payAndBenefits' in res:
                job.payAndBenefits = res.get('payAndBenefits', '')
            elif 'eeoContent' in res:
                # Sometimes additional info is in eeoContent
                eeo = res.get('eeoContent', '')
                if eeo and len(eeo) > 50:  # Only use if it has meaningful content
                    job.payAndBenefits = eeo
            else:
                job.payAndBenefits = None

            logger.debug(f"Successfully fetched details for job {job.positionId}")
            return job
